...
因此 Manager 类名与 GameObject 名一致（`typeof(T).Name`）。空物体上的附加组件由 `typeof` 传入，例如 `AudioListener`。
...
| `SdkManager` | 平台 SDK、平台登录、本地/云读写入口、平台用户信息同步（`SyncPlatformUserInfo` / `TryGetPlatformUserInfo`，`authStateCallBack` 上报 `PlatformAuthStates`（`Invariable` 顶层枚举）、`userInfoCallBack` 仅资料结果，授权按钮显隐由业务层负责）、授权锚点（微信：`SyncPlatformUserInfo` 创建原生按钮，`DestroyPlatformUserInfoButton` 销毁；抖音：`RequestPlatformUserInfoAuth` / `DestroyPlatformUserInfoButton`，同样双回调）、键盘、广告、分享、适配 |
...
`prefabPath` 传路径或文件名均可，内部取文件名并去掉 `.prefab`。Tips/FloatText 业务封装走 `HotUpdateUtils.OpenTipsPanel` / `ShowFloatText`（内部仍调用 `HotUpdateUtils.OpenUIPrefabPanel`）。
...
3. 若同名页面正在加载中则忽略本次打开；
4. 加载地址 `Prefabs_{页面名}`；
5. 检查预制体已挂 `UIPanel`，缺失则报错返回；
6. 实例化到 `UI_Root/Canvas_{layer}/Ts_Panel`；
7. 将其作为 `UIPanel` 注册；
8. 调用回调。
...
- Manager GameObject 创建（`CreateManagerInstance<T>`，附加组件传入 `typeof`）；
- 文件大小格式化（`FormatFileByteSize`，与 `ConfigUtils.FormatFileByteSize` 重复实现）；
- HTML 颜色解析。

`HotUpdateUtils` 为普通类，成员均为静态。主要能力：UI 页面打开（`OpenUIPrefabPanel`）；Tips/FloatText 业务封装；面板父节点按 layer 选择 `InvariableConst.UIPanelPath_0..3`。
